...
const retryWithBackoff = async <T>(
  ...
  try {
    ...
    // Check for Rate Limit (429) or Quota Exhaustion
    const isRateLimit = 
      error?.status === 429 || 
      error?.code === 429 || 
...
export const generateResponse = async (
  ...
  try {
    ...
    const result = await retryWithBackoff(() => chat.sendMessage({ message: prompt }));
    return result.text || "I couldn't generate a response. Please try again.";
  } catch (error) {
    console.error("Gemini API Error:", error);
    return "I'm currently experiencing high traffic. Please try again in a few seconds.";
  }
};
...
export const analyzeToolRequest = async (toolName: string, input: string): Promise<string> => {
  ...
  try {
    ...
    return "Error analyzing request. High traffic, please retry.";
  }
};
